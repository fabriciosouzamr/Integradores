...
public static class BancoDados
{
  ...
  public class clsBancoDados
  {
        ...
        case BancoDados_Constantes.const_TipoBancoDados_MySql:
          {
            ...
            { sRet = "date_add(now(), interval " + iDias.ToString() + " day)"; }
                                    ...
                                    clsCampo[] oParametro = null)
    {
      ...
      foreach (DataRow oRow in oData.Rows)
      {
        ...
        {
          FNC_Str_Adicionar(ref sRet, "'" + oRow[0].ToString().Trim() + "'", ",");
        }
        else
        {
          FNC_Str_Adicionar(ref sRet, oRow[0].ToString(), ",");
        }
      }
                                               ...
                                               string sDs_log)
    {
      ...
      try
      {
        DBSQL_Log_Gravar(sAplicativo, sPartner, sCOD_PUXADA, sTIPO_REGISTRO, sTIPO_CONSULTA, sCodigoConexaoOrigem,
                         sCdServico, 0, LogTipo.ErroNoBancoDados, iId_Tarefa, sDs_log);
      ...
      {
        DBSQL_Log_Gravar(sAplicativo, sPartner, sCOD_PUXADA, sTIPO_REGISTRO, sTIPO_CONSULTA, sCodigoConexaoOrigem,
                         sCdServico, 0, LogTipo.ErroNoBancoDados, iId_Tarefa, Ex.Message);
      }
  ...
  class clsMySql
  {
                                ...
                                clsMySql_Campo[] oParametro = null)
    {
      ...
      {
        Erro_Setar(ex.Message);
        oBancoDados.DBSQL_Log_Gravar("", "", "", "", "", "", "", 0, LogTipo.ErroNoBancoDados, 0, ex.Message);
      }
    ...
    public static string DBDataToString(DateTime dData)
    {
      ...
      sData = FNC_ZerosEsquerda(dData.Day.ToString().Trim(), 2) + "/" +
              FNC_ZerosEsquerda(dData.Month.ToString().Trim(), 2) + "/" +
              FNC_ZerosEsquerda(dData.Year.ToString().Trim(), 4);
